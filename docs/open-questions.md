...
| TQ-009 | Private reaction persistence  | Persist intents separately, target the window's opening revision, acknowledge privately, and advance public/game state only on resolution                                                 | M5          | open        |
...
| TQ-011 | Protocol deployment overlap   | Client and Worker ship together and accept only protocol v1; an incompatible future wire major requires an explicit dual-reading deployment if overlap becomes necessary; see ADR 0011    | M3          | accepted    |
