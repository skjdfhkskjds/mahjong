...
ADR 0017 coalesces the prelaunch gameplay contract into protocol v1. The
client and Worker ship together with content-hashed assets, and unsupported
versions are rejected and closed.

`storageSchemaVersion` is the `TableRoom` SQLite schema version. The complete prelaunch layout is schema version 1; fresh storage creates it directly. Unknown versions fail closed. A permanent complete-v1 fixture proves recovery.
