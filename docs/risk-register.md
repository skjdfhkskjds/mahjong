...
| R-016 | OAuth access token and app session are conflated or retained unsafely         | Low        | Critical | ADR 0009 separates the memory-only SDK token from the signed app cookie; retain boundary tests and credentialed proxy smoke tests | Security/platform   |
